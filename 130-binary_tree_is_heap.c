 ...
 * it has to be the same quantity of levels in left_node as right_node, and also
...
int tree_is_perfect(const binary_tree_t *tree)
{
	...
	{
		length = 1 + tree_is_perfect(tree->left);
		row = 1 + tree_is_perfect(tree->right);
		if (row == length && row != 0 && length != 0)
			return (row);
		return (0);
	}
