...
{
	size_t height_len, height_row, balancer;
	...
	if (tree != NULL)
	{
		...
		height_len = binary_tree_height(tree->left);
		height_row = binary_tree_height(tree->right);
		balancer=height_len > height_row ? height_len - height_row : height_row - height_len;
		if (balancer > 1)
		{
			return (0);
		}
